...
for k in range(0,5):
    ...
    for file in json_files:
        with open(file, 'r') as f:
            ...
            data['Ridge'] = [ round(elem, 2) for elem in data['Ridge'] ]
            data['Ligament'] = [ round(elem, 2) for elem in data['Ligament'] ]
            data['SL'] = [ round(elem, 2) for elem in data['SL'] ]
            for item in data:
    ...
    with open(os.path.join(dirfolder, team + '_output_v4_symdist.csv'), 'w', newline='') as f:
        ...
        for file in json_files:
            with open(file, 'r') as f1:
                data = json.load(f1)
                data['distsym_r_l_SL'] = [ round(elem, 2) for elem in data['distsym_r_l_SL'] ]
                # data['Ligament'] = [ round(elem, 2) for elem in data['Ligament'] ]
                # data['SL'] = [ round(elem, 2) for elem in data['SL'] ]
                
                data['distsym_r_l_SL'] = [1 if i>1 else i for i in data['distsym_r_l_SL']]
                Prec = str(data['distsym_r_l_SL'][0]) + '/'+ str(data['distsym_r_l_SL'][1])+ '/'+ str(data['distsym_r_l_SL'][2])
                # DSC_score = str(data['Ridge'][2])+ '/'+ str(data['Ligament'][2])+ '/'+ str(data['SL'][2])
           
                P.append(Prec)
                # DSC.append(DSC_score)
                
                Pr.append(data['distsym_r_l_SL'][0])

                Psl.append(data['distsym_r_l_SL'][2])
                
                
                if data['patientName'] == 'patient4_21' or data['patientName'] == 'patient4_22':
                    print('No ligament present - NA precision and DSC')
                else:
                    Pl.append(data['distsym_r_l_SL'][1])
        ...
        df = pd.DataFrame({'SymDist':P})
        df.to_csv(os.path.join(dirfolder, team + '_output_SymDist_RLSL.csv'))  
        
        # df.to_csv(os.path.join(dirfolder, team + '_output_DSC_RLSL.csv'))  
        # df.to_csv(os.path.join(dirfolder, team + '_output_v4_chamfer_Chamfer_RL.csv'))  
        # print(R_chamfer)
        # print(L_chamfer)
    print ('----------------------', team)
    import numpy as np
    print('average symmetric distance values for ridge {}, ligament {} and SL {}'.format(round(np.mean(Pr), 2), round(np.mean(Pl),2), round(np.mean(Psl),2))) 
    # print('average DSC values for ridge {}, ligament {} and SL {}'.format(round(np.mean(Dr),2), round(np.mean(Dl),2), round(np.mean(Dsl),2)))
    
    
    print('Data has been written to output.csv file.')
    
    # ridge 0.27312499999999995, ligament 0.359375 and SL 0.46375
    # average DSC values for ridge 0.01625, ligament 0.376875 and SL 0.4975
