...
for k in range(0,5):
    ...
    with open(os.path.join(dirfolder, team + '_output_v4_DSC.csv'), 'w', newline='') as f:
        ...
        for file in json_files:
            with open(file, 'r') as f1:
                ...
                Prec = str(data['Ridge'][0]) + '/'+ str(data['Ligament'][0])+ '/'+ str(data['SL'][0])
                DSC_score = str(data['Ridge'][2])+ '/'+ str(data['Ligament'][2])+ '/'+ str(data['SL'][2])
            
                P.append(Prec)
                DSC.append(DSC_score)
                
                Pr.append(data['Ridge'][0])

                Psl.append(data['SL'][0])
                Dr.append(data['Ridge'][2])

                Dsl.append(data['SL'][2])
                
                if data['patientName'] == 'patient4_21' or data['patientName'] == 'patient4_22':
                    print('No ligament present - NA precision and DSC')
                else:
                    Pl.append(data['Ligament'][0])
                    Dl.append(data['Ligament'][2])
                ...
                for item in data:
                    ...
                    for header in all_headers:
                        ...
                        # else:
                            # row.append('')
                writer.writerow(row)
        df = pd.DataFrame({'Precision':P, 'DSC':DSC})
    ...
    print ('----------------------', team)
    import numpy as np
    print('average precision values for ridge {}, ligament {} and SL {}'.format(round(np.mean(Pr), 2), round(np.mean(Pl),2), round(np.mean(Psl),2))) 
